...
const TopChartCard = ({ song, i, isPlaying, activeSong, handlePauseClick, handlePlayClick }) => (
  <div className="w-full flex flex-row items-center hover:bg-[#CF7500] py-2 p-4 rounded-lg cursor-pointer mb-2">
    <h3 className="fot-bold text-base text-[#F4F4F4] mr-3">{i + 1}</h3>
    <div className="flex-1 flex flex-row justify-between items-center">
      <img src={song?.images?.coverart} alt={song?.title} className="w-20 h-20 rounded-lg" />
      <div className="flex-1 flex flex-col justify-center mx-3">
        <Link to={`/songs/${song.key}`}>
          <p className="text-xl font-bold text-[#F4F4F4]">{song.title}</p>
        </Link>
    ...
    <div ref={divRef} className="xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] max-w-full flex flex-col">
      ...
      <div className="w-full flex flex-col">
        <div className="flex flex-row justify-between items-center">
          <h2 className="text-[#F0A500] font-bold text-2xl">Top Artists</h2>
          <Link to="/top-artists">
            <p className="text-[#F0A500] text-base cursor-pointer">See more</p>
          </Link>
        </div>
      ...
      <div className="w-full flex flex-col mt-9">
        <div className="flex flex-row justify-between items-center">
          <h2 className="text-[#F0A500] font-bold text-2xl">Top Charts</h2>
          <Link to="/top-charts">
            <p className="text-[#F0A500] text-base cursor-pointer">See more</p>
          </Link>
        </div>
